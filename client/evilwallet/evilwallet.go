...
var (
	...
	faucetBalance      = devnetvm2.NewColoredBalances(map[devnetvm2.Color]uint64{
		devnetvm2.ColorIOTA: uint64(faucetTokensPerRequest),
	})
)
...
func (e *EvilWallet) RequestFundsFromFaucet(options ...FaucetRequestOption) (err error, initWallet *Wallet) {
	...
	if buildOptions.outputAliasName != "" {
		input := devnetvm2.NewUTXOInput(outputID)
...
func (e *EvilWallet) handleInputOutputDuringSplitOutputs(splitNumber int, inputWallet *Wallet, inputAddr string) (input utxo.OutputID, outputs []*OutputOption) {
	...
	evilInput.Balance.ForEach(func(color devnetvm2.Color, balance uint64) bool {
...
func (e *EvilWallet) PrepareCustomConflicts(conflictsMaps []ConflictSlice) (conflictBatch [][]*devnetvm2.Transaction, err error) {
	for _, conflictMap := range conflictsMaps {
		var txs []*devnetvm2.Transaction
...
func (e *EvilWallet) SendCustomConflicts(conflictsMaps []ConflictSlice) (err error) {
	...
	for _, txs := range conflictBatch {
		...
		for i, tx := range txs {
			wg.Add(1)
			go func(clt Client, tx *devnetvm2.Transaction) {
				defer wg.Done()
				if err = RateSetterSleep(clt, true); err != nil {
					return
				}
...
func (e *EvilWallet) CreateTransaction(options ...Option) (tx *devnetvm2.Transaction, err error) {
	...
	tx, err = e.makeTransaction(devnetvm2.NewInputs(inputs...), devnetvm2.NewOutputs(outputs...), buildOptions.inputWallet)
	if err != nil {
		return nil, err
	}
...
func (e *EvilWallet) addOutputsToOutputManager(tx *devnetvm2.Transaction, outWallet, tmpWallet *Wallet, tempAddresses map[devnetvm2.Address]types.Empty) {
...
func (e *EvilWallet) registerOutputAliases(outputs devnetvm2.Outputs, addrAliasMap map[devnetvm2.Address]string) {
	...
	for _, output := range outputs {
		...
		input := devnetvm2.NewUTXOInput(output.ID())
		e.aliasManager.AddInputAlias(input, addrAliasMap[output.Address()])
	}
	return
}

func (e *EvilWallet) prepareInputs(buildOptions *Options) (inputs []devnetvm2.Input, err error) {
	if buildOptions.areInputsProvidedWithoutAliases() {
		for _, out := range buildOptions.inputs {
			inputs = append(inputs, devnetvm2.NewUTXOInput(out))
		}
		return
	}
...
func (e *EvilWallet) prepareOutputs(buildOptions *Options, tempWallet *Wallet) (outputs []devnetvm2.Output,
	addrAliasMap map[devnetvm2.Address]string, tempAddresses map[devnetvm2.Address]types.Empty, err error,
) {
	if buildOptions.areOutputsProvidedWithoutAliases() {
		for _, balance := range buildOptions.outputs {
			output := devnetvm2.NewSigLockedColoredOutput(balance, buildOptions.outputWallet.Address().Address())
...
func (e *EvilWallet) matchInputsWithAliases(buildOptions *Options) (inputs []devnetvm2.Input, err error) {
	// get inputs by alias
	for inputAlias := range buildOptions.aliasInputs {
		...
		if !ok {
			...
			in = devnetvm2.NewUTXOInput(out.OutputID)
...
func (e *EvilWallet) matchOutputsWithAliases(buildOptions *Options, tempWallet *Wallet) (outputs []devnetvm2.Output,
	addrAliasMap map[devnetvm2.Address]string, tempAddresses map[devnetvm2.Address]types.Empty, err error,
) {
	...
	tempAddresses = make(map[devnetvm2.Address]types.Empty)
	addrAliasMap = make(map[devnetvm2.Address]string)
	for alias, balance := range buildOptions.aliasOutputs {
		var addr devnetvm2.Address
		...
		outputs = append(outputs, devnetvm2.NewSigLockedColoredOutput(balance, addr))
...
func (e *EvilWallet) prepareRemainderOutput(buildOptions *Options, outputs []devnetvm2.Output) (alias string, remainderOutput devnetvm2.Output, added bool) {
	inputBalance := uint64(0)

	var remainderAddress devnetvm2.Address
	for inputAlias := range buildOptions.aliasInputs {
		...
		output.Balance.ForEach(func(color devnetvm2.Color, balance uint64) bool {
			inputBalance += balance
			return true
		})
		if alias == "" {
	...
	for _, input := range buildOptions.inputs {
		...
		inputDetails.Balance.ForEach(func(color devnetvm2.Color, balance uint64) bool {
			inputBalance += balance
			return true
		})
	...
	for _, o := range outputs {
		o.Balances().ForEach(func(color devnetvm2.Color, balance uint64) bool {
	...
	if outputBalance < inputBalance {
		remainderOutput = devnetvm2.NewSigLockedColoredOutput(devnetvm2.NewColoredBalances(map[devnetvm2.Color]uint64{
			devnetvm2.ColorIOTA: inputBalance - outputBalance,
		}), remainderAddress)
		added = true
	}
...
func (e *EvilWallet) updateOutputBalances(buildOptions *Options) (err error) {
	...
	if !buildOptions.isBalanceProvided() {
		...
		for out := range buildOptions.aliasOutputs {
			buildOptions.aliasOutputs[out] = devnetvm2.NewColoredBalances(map[devnetvm2.Color]uint64{
				devnetvm2.ColorIOTA: balances[i],
			})
			i++
		}
	}
...
func (e *EvilWallet) makeTransaction(inputs devnetvm2.Inputs, outputs devnetvm2.Outputs, w *Wallet) (tx *devnetvm2.Transaction, err error) {
	txEssence := devnetvm2.NewTransactionEssence(0, time.Now(), identity.ID{}, identity.ID{}, inputs, outputs)
	unlockBlocks := make([]devnetvm2.UnlockBlock, len(txEssence.Inputs()))
	for i, input := range txEssence.Inputs() {
		...
		unlockBlocks[i] = devnetvm2.NewSignatureUnlockBlock(wallet.Sign(addr, txEssence))
	}
	return devnetvm2.NewTransaction(txEssence, unlockBlocks), nil
}

func (e *EvilWallet) getAddressFromInput(input devnetvm2.Input) (addr devnetvm2.Address, err error) {
	typeCastedInput, ok := input.(*devnetvm2.UTXOInput)
	if !ok {
...
func (e *EvilWallet) PrepareCustomConflictsSpam(scenario *EvilScenario) (txs [][]*devnetvm2.Transaction, allAliases ScenarioAlias, err error) {
...
func (e *EvilWallet) AwaitInputsSolidity(inputs devnetvm2.Inputs, clt Client) (allSolid bool) {
...
func (e *EvilWallet) SetTxOutputsSolid(outputs devnetvm2.Outputs, clientID string) {
...
func (e *EvilWallet) AddReuseOutputsToThePool(outputs devnetvm2.Outputs) {
	for _, out := range outputs {
