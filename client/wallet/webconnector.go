...
func (webConnector WebConnector) GetAllowedPledgeIDs() (pledgeIDMap map[manamodels.Type][]string, err error) {
	res, err := webConnector.client.GetAllowedManaPledgeNodeIDs()
	if err != nil {
		return
	}
	pledgeIDMap = map[manamodels.Type][]string{
		manamodels.AccessMana:    res.Access.Allowed,
		manamodels.ConsensusMana: res.Consensus.Allowed,
	}
