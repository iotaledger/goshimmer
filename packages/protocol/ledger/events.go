...
var NewEvents = event.NewLinkableEvents(func(self *Events) (linker func(*Events)) {
	self.TransactionStored = event.NewLinkable[*TransactionStoredEvent, Events]()
	self.TransactionBooked = event.NewLinkable[*TransactionBookedEvent, Events]()
	self.TransactionInclusionUpdated = event.NewLinkable[*TransactionInclusionUpdatedEvent, Events]()
	self.TransactionAccepted = event.NewLinkable[*TransactionAcceptedEvent, Events]()
	self.TransactionRejected = event.NewLinkable[*TransactionRejectedEvent, Events]()
	self.TransactionForked = event.NewLinkable[*TransactionForkedEvent, Events]()
	self.TransactionConflictIDUpdated = event.NewLinkable[*TransactionConflictIDUpdatedEvent, Events]()
	self.TransactionInvalid = event.NewLinkable[*TransactionInvalidEvent, Events]()
	self.Error = event.NewLinkable[error, Events]()

	return func(newTarget *Events) {
		self.TransactionStored.LinkTo(newTarget.TransactionStored)
		self.TransactionBooked.LinkTo(newTarget.TransactionBooked)
		self.TransactionInclusionUpdated.LinkTo(newTarget.TransactionInclusionUpdated)
		self.TransactionAccepted.LinkTo(newTarget.TransactionAccepted)
		self.TransactionRejected.LinkTo(newTarget.TransactionRejected)
		self.TransactionForked.LinkTo(newTarget.TransactionForked)
		self.TransactionConflictIDUpdated.LinkTo(newTarget.TransactionConflictIDUpdated)
		self.TransactionInvalid.LinkTo(newTarget.TransactionInvalid)
