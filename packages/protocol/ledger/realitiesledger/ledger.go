...
func NewProvider(opts ...options.Option[RealitiesLedger]) module.Provider[*engine.Engine, ledger.Ledger] {
...
func (l *RealitiesLedger) Initialize(workerPool *workerpool.WorkerPool, storage *storage.Storage) {
	...
	l.events.TransactionBooked.Hook(func(event *ledger.TransactionBookedEvent) {
		l.processConsumingTransactions(event.Outputs.IDs())
	}, event.WithWorkerPool(l.workerPool))
	l.events.TransactionInvalid.Hook(func(event *ledger.TransactionInvalidEvent) {
...
func (l *RealitiesLedger) Events() *ledger.Events {
	return l.events
}
...
func (l *RealitiesLedger) Storage() ledger.Storage {
	return l.storage
}

func (l *RealitiesLedger) Utils() ledger.Utils {
	return l.utils
}
...
var _ ledger.Ledger = new(RealitiesLedger)
...
func (l *RealitiesLedger) SetTransactionInclusionSlot(id utxo.TransactionID, inclusionSlot slot.Index) {
	l.storage.CachedTransactionMetadata(id).Consume(func(metadata *ledger.TransactionMetadata) {
		...
		for it := metadata.OutputIDs().Iterator(); it.HasNext(); {
			l.storage.CachedOutputMetadata(it.Next()).Consume(func(outputMetadata *ledger.OutputMetadata) {
				outputMetadata.SetInclusionSlot(inclusionSlot)
			})
		}

		if previousInclusionSlot != 0 {
			l.events.TransactionInclusionUpdated.Trigger(&ledger.TransactionInclusionUpdatedEvent{
				TransactionID:         id,
...
func (l *RealitiesLedger) triggerAcceptedEvent(txMetadata *ledger.TransactionMetadata) (triggered bool) {
	...
	l.storage.CachedTransaction(txMetadata.ID()).Consume(func(tx utxo.Transaction) {
		for it := l.utils.ResolveInputs(tx.Inputs()).Iterator(); it.HasNext(); {
			inputID := it.Next()
			l.storage.CachedOutputMetadata(inputID).Consume(func(outputMetadata *ledger.OutputMetadata) {
	...
	transactionEvent := &ledger.TransactionEvent{
		Metadata:       txMetadata,
		CreatedOutputs: make([]*ledger.OutputWithMetadata, 0),
		SpentOutputs:   make([]*ledger.OutputWithMetadata, 0),
	}
	...
	for it := txMetadata.OutputIDs().Iterator(); it.HasNext(); {
		...
		l.storage.CachedOutputMetadata(outputID).Consume(func(outputMetadata *ledger.OutputMetadata) {
			...
			l.storage.CachedOutput(outputID).Consume(func(output utxo.Output) {
				transactionEvent.CreatedOutputs = append(transactionEvent.CreatedOutputs, ledger.NewOutputWithMetadata(
		...
		for it := l.utils.ResolveInputs(tx.Inputs()).Iterator(); it.HasNext(); {
			...
			l.storage.CachedOutputMetadata(inputID).Consume(func(outputMetadata *ledger.OutputMetadata) {
				l.storage.CachedOutput(inputID).Consume(func(output utxo.Output) {
					transactionEvent.SpentOutputs = append(transactionEvent.SpentOutputs, ledger.NewOutputWithMetadata(
						outputMetadata.InclusionSlot(),
						inputID,
						output,
						outputMetadata.ConsensusManaPledgeID(),
...
func (l *RealitiesLedger) triggerRejectedEvent(txMetadata *ledger.TransactionMetadata) (triggered bool) {
...
func (l *RealitiesLedger) triggerRejectedEventLocked(txMetadata *ledger.TransactionMetadata) (triggered bool) {
	l.mutex.Lock(txMetadata.ID())
	defer l.mutex.Unlock(txMetadata.ID())
...
func (l *RealitiesLedger) propagateAcceptanceToIncludedTransactions(txID utxo.TransactionID) {
	l.storage.CachedTransactionMetadata(txID).Consume(func(txMetadata *ledger.TransactionMetadata) {
		if !l.triggerAcceptedEvent(txMetadata) {
			return
		}

		l.utils.WalkConsumingTransactionMetadata(txMetadata.OutputIDs(), func(consumingTxMetadata *ledger.TransactionMetadata, walker *walker.Walker[utxo.OutputID]) {
...
func (l *RealitiesLedger) propagatedRejectionToTransactions(conflict *conflictdag.Conflict[utxo.TransactionID, utxo.OutputID]) {
	l.storage.CachedTransactionMetadata(conflict.ID()).Consume(func(txMetadata *ledger.TransactionMetadata) {
