...
func (b *booker) bookTransaction(ctx context.Context, tx utxo.Transaction, txMetadata *ledger.TransactionMetadata, inputsMetadata *ledger.OutputsMetadata, consumers []*ledger.Consumer, outputs *utxo.Outputs) {
	...
	lo.ForEach(consumers, func(consumer *ledger.Consumer) { consumer.SetBooked() })

	b.ledger.Events().TransactionBooked.Trigger(&ledger.TransactionBookedEvent{
...
func (b *booker) inheritConflictIDs(ctx context.Context, txID utxo.TransactionID, inputsMetadata *ledger.OutputsMetadata) (inheritedConflictIDs *advancedset.AdvancedSet[utxo.TransactionID]) {
...
func (b *booker) determineConflictDetails(txID utxo.TransactionID, inputsMetadata *ledger.OutputsMetadata) (conflictingInputIDs utxo.OutputIDs, consumersToFork utxo.TransactionIDs) {
	conflictingInputIDs = utxo.NewOutputIDs()
	consumersToFork = utxo.NewTransactionIDs()

	_ = inputsMetadata.ForEach(func(outputMetadata *ledger.OutputMetadata) error {
...
func (b *booker) forkTransaction(ctx context.Context, txID utxo.TransactionID, outputsSpentByConflictingTx utxo.OutputIDs) (confirmationState confirmation.State) {
	b.ledger.Utils().WithTransactionAndMetadata(txID, func(tx utxo.Transaction, txMetadata *ledger.TransactionMetadata) {
		...
		b.ledger.Events().TransactionForked.Trigger(&ledger.TransactionForkedEvent{
			TransactionID:   txID,
...
func (b *booker) propagateForkedConflictToFutureCone(ctx context.Context, outputIDs utxo.OutputIDs, forkedConflictID utxo.TransactionID, previousParentConflicts *advancedset.AdvancedSet[utxo.TransactionID]) {
	b.ledger.Utils().WalkConsumingTransactionMetadata(outputIDs, func(consumingTxMetadata *ledger.TransactionMetadata, walker *walker.Walker[utxo.OutputID]) {
...
func (b *booker) updateConflictsAfterFork(ctx context.Context, txMetadata *ledger.TransactionMetadata, forkedConflictID utxo.TransactionID, previousParents *advancedset.AdvancedSet[utxo.TransactionID]) (updated bool) {
	...
	b.ledger.Storage().CachedOutputsMetadata(txMetadata.OutputIDs()).Consume(func(outputMetadata *ledger.OutputMetadata) {
		outputMetadata.SetConflictIDs(newConflicts)
	})

	txMetadata.SetConflictIDs(newConflicts)

	b.ledger.Events().TransactionConflictIDUpdated.Trigger(&ledger.TransactionConflictIDUpdatedEvent{
		TransactionID:      txMetadata.ID(),
