...
func TestAliasOutput_validateTransition(t *testing.T) {
	...
	t.Run("CASE: Gov update, delegation without delegation lock", func(t *testing.T) {
		...
		tx := model.NewStorable[utxo2.TransactionID, Transaction](&transactionModel{
	...
	t.Run("CASE: Gov update, delegation with delegation lock", func(t *testing.T) {
		...
		err := prev.validateTransition(next, model.NewStorable[utxo2.TransactionID, Transaction](&transactionModel{Essence: model.NewImmutable[TransactionEssence](&transactionEssenceModel{Timestamp: timeLock.Add(time.Second)})}))
		assert.NoError(t, err)
	...
	t.Run("CASE: State update, delegation without time-lock", func(t *testing.T) {
		prev := dummyAliasOutput().WithDelegation()
		next := prev.NewAliasOutputNext(false)
		err := prev.validateTransition(next, model.NewStorable[utxo2.TransactionID, Transaction](&transactionModel{Essence: model.NewImmutable[TransactionEssence](&transactionEssenceModel{Timestamp: time.Now()})}))
		assert.NoError(t, err)
	})
...
func TestAliasOutput_UnlockValid(t *testing.T) {
	...
	t.Run("CASE: Unsupported unlock block", func(t *testing.T) {
		...
		inputs := NewInputs(
			NewUTXOInput(utxo2.NewOutputID(utxo2.TransactionID{}, 0)),
		)
		outputs := NewOutputs(
...
func TestExtendedLockedOutput_UnlockValid(t *testing.T) {
	...
	t.Run("CASE: Happy path, unlocked by alias", func(t *testing.T) {
		...
		inputToUnlockMapping := make(map[utxo2.OutputID]UnlockBlock)
	...
	t.Run("CASE: Unsupported unlock block", func(t *testing.T) {
		...
		valid, err := input.UnlockValid(model.NewStorable[utxo2.TransactionID, Transaction](&transactionModel{Essence: new(TransactionEssence)}), unlockBlock, Outputs{input})
		t.Log(err)
		assert.Error(t, err)
		assert.False(t, valid)
	})
...
func randOutputID() (randomOutputID utxo2.OutputID) {
