...
func NewTestFramework(test *testing.T, workers *workerpool.Group, epochTimeProvider *epoch.TimeProvider, optsScheduler ...options.Option[Scheduler]) *TestFramework {
	t := &TestFramework{
		test:           test,
		workers:        workers,
	...
	t.engine = engine.New(workers.CreateGroup("Engine"), t.storage, dpos.NewProvider(), mana1.NewProvider(), epochTimeProvider)
	test.Cleanup(func() {
	...
	t.Scheduler = New(t.Tangle.BlockDAG.Instance.EvictionState, epochTimeProvider, t.mockAcceptance.IsBlockAccepted, t.ManaMap, t.TotalMana, optsScheduler...)
...
func (t *TestFramework) CreateSchedulerBlock(opts ...options.Option[models.Block]) *Block {
	...
	if err := blk.DetermineID(t.Tangle.Instance.BlockDAG.EpochTimeProvider); err != nil {
