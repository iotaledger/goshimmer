...
func TestAliasOutput_NewAliasOutputMint(t *testing.T) {
	t.Run("CASE: Happy path", func(t *testing.T) {
		...
		require.True(t, ok)
		require.Equal(t, DustThresholdAliasOutputIOTA, iotaBal)
		require.True(t, alias.GetStateAddress().Equals(stateAddy))
		require.Nil(t, alias.GetImmutableData())
	})
	...
	t.Run("CASE: Happy path with immutable data", func(t *testing.T) {
		...
		require.Equal(t, DustThresholdAliasOutputIOTA, iotaBal)
		require.True(t, alias.GetStateAddress().Equals(stateAddy))
		require.True(t, bytes.Equal(alias.GetImmutableData(), data))
	})
...
func TestAliasOutput_NewAliasOutputNext(t *testing.T) {
	...
	t.Run("CASE: Happy path, no governance update", func(t *testing.T) {
		...
		require.True(t, originAlias.GetAliasAddress().Equals(nextAlias.GetAliasAddress()))
		require.True(t, originAlias.GetStateAddress().Equals(nextAlias.GetStateAddress()))
		require.True(t, originAlias.GetGoverningAddress().Equals(nextAlias.GetGoverningAddress()))
		// OutputID is actually irrelevant here
		require.True(t, bytes.Equal(lo.PanicOnErr(nextAlias.ID().Bytes()), lo.PanicOnErr(originAlias.ID().Bytes())))
		require.Equal(t, originAlias.Balances().Bytes(), nextAlias.Balances().Bytes())
		require.Equal(t, originAlias.GetStateIndex()+1, nextAlias.GetStateIndex())
		require.Equal(t, originAlias.GetStateData(), nextAlias.GetStateData())
		require.Equal(t, originAlias.GetGovernanceMetadata(), nextAlias.GetGovernanceMetadata())
		require.Equal(t, originAlias.GetImmutableData(), nextAlias.GetImmutableData())
...
func TestAliasOutput_validateTransition(t *testing.T) {
	...
	t.Run("CASE: Gov update, delegation without delegation lock", func(t *testing.T) {
		...
		err := prev.validateTransition(next, tx)
		assert.NoError(t, err)
	})
...
func TestAliasOutput_unlockedGovernanceByAliasIndex(t *testing.T) {
	governingAliasStateKeyPair := ed25519.GenerateKeyPair()
	governingAliasStateAddress := NewED25519Address(governingAliasStateKeyPair.PublicKey)
	...
	aliasStateKeyPair := ed25519.GenerateKeyPair()
	...
	t.Run("CASE: Happy path", func(t *testing.T) {
		...
		unlocks[indexOfGoverningAliasInput] = NewSignatureUnlockBlock(NewED25519Signature(governingAliasStateKeyPair.PublicKey, governingAliasStateKeyPair.PrivateKey.Sign(lo.PanicOnErr(essence.Bytes()))))
...
func TestAliasOutput_UnlockValid(t *testing.T) {
	...
	t.Run("CASE: Unsupported unlock block", func(t *testing.T) {
		...
		txEssence := NewTransactionEssence(0, time.Time{}, issuerIdentity.ID(), issuerIdentity.ID(), inputs, outputs)
		unlockBlocks := UnlockBlocks{
			NewSignatureUnlockBlock(NewED25519Signature(issuerKeyPair.PublicKey, issuerKeyPair.PrivateKey.Sign(lo.PanicOnErr(txEssence.Bytes())))),
		}
...
func TestExtendedLockedOutput_UnlockValid(t *testing.T) {
	...
	t.Run("CASE: Happy path, unlocked by alias", func(t *testing.T) {
		...
		inputToUnlockMapping := make(map[utxo.OutputID]UnlockBlock)
