...
func TestOTV_Track(t *testing.T) {
	...
	tf.VirtualVoting.ValidateMarkerVoters(lo.MergeMaps(initialMarkerVotes, map[markers.Marker]*set.AdvancedSet[identity.ID]{
	...
	tf.VirtualVoting.ValidateConflictVoters(lo.MergeMaps(initialConflictVotes, map[utxo.TransactionID]*set.AdvancedSet[identity.ID]{
