...
func NewCommitmentFactory(genesisCommitment *commitment.Commitment, chainStorage *chainstorage.ChainStorage, snapshotDepth int) *CommitmentFactory {
	return &CommitmentFactory{
		...
		stateRootTree: smt.NewSparseMerkleTree(
			lo.PanicOnErr(chainStorage.StateTreeStorage().WithExtendedRealm([]byte{prefixStateTreeNodes})),
			lo.PanicOnErr(chainStorage.StateTreeStorage().WithExtendedRealm([]byte{prefixStateTreeValues})),
			lo.PanicOnErr(blake2b.New256(nil)),
		),
		manaRootTree: smt.NewSparseMerkleTree(
			lo.PanicOnErr(chainStorage.ManaTreeStorage().WithExtendedRealm([]byte{prefixManaTreeNodes})),
			lo.PanicOnErr(chainStorage.ManaTreeStorage().WithExtendedRealm([]byte{prefixManaTreeValues})),
			lo.PanicOnErr(blake2b.New256(nil)),
		),
	}
}
