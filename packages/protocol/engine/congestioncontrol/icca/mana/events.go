...
type ManaEvents struct {
	...
	Pledged *event.Linkable[*PledgedEvent, ManaEvents, *ManaEvents]
	// Fired when mana was revoked from a node.
	Revoked *event.Linkable[*RevokedEvent, ManaEvents, *ManaEvents]
	// Fired when mana of a node was updated.
	Updated *event.Linkable[*UpdatedEvent, ManaEvents, *ManaEvents]

	event.LinkableCollection[ManaEvents, *ManaEvents]
}

// NewEvents contains the constructor of the Events object (it is generated by a generic factory).
var NewEvents = event.NewLinkableEvents(func(self *ManaEvents) (linker func(*ManaEvents)) {
	self.Pledged = event.NewLinkable[*PledgedEvent, ManaEvents]()
	self.Revoked = event.NewLinkable[*RevokedEvent, ManaEvents]()
	self.Updated = event.NewLinkable[*UpdatedEvent, ManaEvents]()

	return func(newTarget *ManaEvents) {
		self.Pledged.LinkTo(newTarget.Pledged)
		self.Revoked.LinkTo(newTarget.Revoked)
		self.Updated.LinkTo(newTarget.Updated)
	}
})
