...
func (u *UtxoDBLedger) GetHighGoFTransaction(txid ledgerstate.TransactionID, f func(ret *ledgerstate.Transaction)) (found bool) {
	found = false
	u.tangleInstance.Ledger.Storage.CachedTransactionMetadata(txid).Consume(func(txmeta *ledgerstate.TransactionMetadata) {
		if txmeta.GradeOfFinality() == gof.High {
			found = true
			u.tangleInstance.Ledger.Storage.CachedTransaction(txid).Consume(f)
		}
	})
	return
}
