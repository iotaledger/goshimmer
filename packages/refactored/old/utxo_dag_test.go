...
func TestBookNonConflictingTransaction(t *testing.T) {
	...
	ledgerstate.outputsMetadata(tx).Consume(func(metadata *OutputMetadata) {
...
func TestBookConflictingTransaction(t *testing.T) {
	...
	inputsMetadata := OutputsMetadata{}
	ledgerstate.outputsMetadata(tx1).Consume(func(metadata *OutputMetadata) {
		inputsMetadata = append(inputsMetadata, metadata)
	})
	...
	ledgerstate.outputsMetadata(tx2).Consume(func(metadata *OutputMetadata) {
...
func setupDependencies(t *testing.T) *Ledger {
...
func generateOutput(ledgerstate *Ledger, address Address, index uint16) *SigLockedSingleOutput {
...
func generateOutputs(ledgerstate *Ledger, address Address, branchIDs BranchIDs) (outputs []*SigLockedSingleOutput) {
	i := 0
...
func singleInputTransaction(ledgerstate *Ledger, a, b txvm.wallet, outputToSpend *SigLockedSingleOutput, optionalGradeOfFinality ...gof.GradeOfFinality) (*Transaction, *SigLockedSingleOutput) {
...
// func multipleInputsTransaction(ledgerstate *Ledger, a, b wallet, outputsToSpend []*SigLockedSingleOutput, optionalGradeOfFinality ...gof.GradeOfFinality) *Transaction {
...
func buildTransaction(_ *Ledger, a, b txvm.wallet, outputsToSpend []*SigLockedSingleOutput) *Transaction {
	inputs := make(Inputs, len(outputsToSpend))
	sum := uint64(0)
	for i, outputToSpend := range outputsToSpend {
		inputs[i] = NewUTXOInput(outputToSpend.ID())
