...
func TestLedger_SetBranchConfirmed(t *testing.T) {
	...
	// Mark A as Confirmed
	{
		...
		require.True(t, testFramework.Ledger.BranchDAG.SetBranchConfirmed(branchdag2.NewBranchID(testFramework.Transaction("TXA").ID())))
		...
		assert.Equal(t, branchdag2.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXA")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXB")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXC")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXD")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXH")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXI")))
	}
	...
	// When creating the conflicting TX (G) of the top layer branches F & G are spawned by the fork of G
	{
		...
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXH")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXI")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXF")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXG")))
	}

	require.True(t, testFramework.Ledger.BranchDAG.SetBranchConfirmed(branchdag2.NewBranchID(testFramework.Transaction("TXD").ID())))
	...
	// TX L combines a child (G) of a Rejected branch (C) and a pending branch H, resulting in (G,H)
	{
		...
		assert.Equal(t, branchdag2.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXA")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXB")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXC")))
		assert.Equal(t, branchdag2.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXD")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXH")))
		assert.Equal(t, branchdag2.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXI")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXF")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXG")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXG", "TXH")))
	}

	require.True(t, testFramework.Ledger.BranchDAG.SetBranchConfirmed(branchdag2.NewBranchID(testFramework.Transaction("TXH").ID())))
	...
	// The new TX M should be now booked under G, as branch H confirmed, just G because we don't propagate H further.
	{
		...
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXC")))
		assert.Equal(t, branchdag2.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXD")))
		assert.Equal(t, branchdag2.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXH")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXI")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXF")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXG")))
		assert.Equal(t, branchdag2.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.BranchIDs("TXG", "TXH")))
	}
