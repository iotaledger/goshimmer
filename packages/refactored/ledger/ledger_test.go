...
func TestLedger_SetBranchConfirmed(t *testing.T) {
	...
	// Mark A as Confirmed
	{
		...
		assert.Equal(t, branchdag.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXA")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXB")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXC")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXD")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXH")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXI")))
	}
	...
	// When creating the conflicting TX (G) of the top layer branches F & G are spawned by the fork of G
	{
		...
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXH")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXI")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXF")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXG")))
	}
	...
	// TX L combines a child (G) of a Rejected branch (C) and a pending branch H, resulting in (G,H)
	{
		...
		assert.Equal(t, branchdag.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXA")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXB")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXC")))
		assert.Equal(t, branchdag.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXD")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXH")))
		assert.Equal(t, branchdag.Pending, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXI")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXF")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXG")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXG", "TXH")))
	}
	...
	// The new TX M should be now booked under G, as branch H confirmed, just G because we don't propagate H further.
	{
		...
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXC")))
		assert.Equal(t, branchdag.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXD")))
		assert.Equal(t, branchdag.Confirmed, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXH")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXI")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXF")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXG")))
		assert.Equal(t, branchdag.Rejected, testFramework.Ledger.BranchDAG.InclusionState(testFramework.TransactionIDs("TXG", "TXH")))
	}
