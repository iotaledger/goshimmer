...
func NewFirewall(gossipMgr *gossip2.Manager, autopeering *selection.Protocol, log *logger.Logger) (*Firewall, error) {
	return &Firewall{
		gossipMgr:            gossipMgr,
...
func (f *Firewall) HandleFaultyPeer(peerID identity.ID, details *FaultinessDetails) {
	...
	if nbr.Group == gossip2.NeighborsGroupAuto {
	...
	} else if nbr.Group == gossip2.NeighborsGroupManual {
