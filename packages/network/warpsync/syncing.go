...
func (p *Protocol) RequestEpochBlocks(ei epoch.Index, ec epoch.EC, to ...identity.ID) {
	...
	p.log.Debugw("sent epoch blocks request", "EI", ei, "EC", ec.Base58())
}

func (p *Protocol) SendEpochStarter(ei epoch.Index, ec epoch.EC, blocksCount int, to ...identity.ID) {
...
func (p *Protocol) SendBlocksBatch(ei epoch.Index, ec epoch.EC, blocks []*models.Block, to ...identity.ID) {
...
func (p *Protocol) SendEpochEnd(ei epoch.Index, ec epoch.EC, roots *epoch.CommitmentRoots, to ...identity.ID) {
...
func (p *Protocol) processEpochBlocksRequestPacket(packetEpochRequest *wp.Packet_EpochBlocksRequest, nbr *p2p.Neighbor) {
	...
	p.log.Debugw("received epoch blocks request", "peer", nbr.Peer.ID(), "EI", ei, "EC", ec)
...
func (p *Protocol) processEpochBlocksStartPacket(packetEpochBlocksStart *wp.Packet_EpochBlocksStart, nbr *p2p.Neighbor) {
	...
	p.log.Debugw("received epoch blocks start", "peer", nbr.Peer.ID(), "EI", ei, "blocksCount", epochBlocksStart.GetBlocksCount())
...
func (p *Protocol) processEpochBlocksBatchPacket(packetEpochBlocksBatch *wp.Packet_EpochBlocksBatch, nbr *p2p.Neighbor) {
	...
	p.log.Debugw("received epoch blocks", "peer", nbr.Peer.ID(), "EI", ei, "blocksLen", len(blocksBytes))
...
func (p *Protocol) processEpochBlocksEndPacket(packetEpochBlocksEnd *wp.Packet_EpochBlocksEnd, nbr *p2p.Neighbor) {
	...
	p.log.Debugw("received epoch blocks end", "peer", nbr.Peer.ID(), "EI", ei)
