...
type Manager struct {
	...
	onGossipNeighborRemovedClosure *event.Closure[*gossip2.NeighborRemovedEvent]
	onGossipNeighborAddedClosure   *event.Closure[*gossip2.NeighborAddedEvent]
}

// NewManager initializes a new Manager instance.
func NewManager(gm *gossip2.Manager, local *peer.Local, log *logger.Logger) *Manager {
	m := &Manager{
		gm:                gm,
		local:             local,
		log:               log,
	...
	m.onGossipNeighborRemovedClosure = event.NewClosure(func(event *gossip2.NeighborRemovedEvent) { m.onGossipNeighborRemoved(event.Neighbor) })
	m.onGossipNeighborAddedClosure = event.NewClosure(func(event *gossip2.NeighborAddedEvent) { m.onGossipNeighborAdded(event.Neighbor) })
	return m
}
...
func (m *Manager) Start() {
	m.startOnce.Do(func() {
		m.gm.NeighborsEvents(gossip2.NeighborsGroupManual).NeighborRemoved.Attach(m.onGossipNeighborRemovedClosure)
		m.gm.NeighborsEvents(gossip2.NeighborsGroupManual).NeighborAdded.Attach(m.onGossipNeighborAddedClosure)
		m.isStarted.Set()
	})
}
...
func (m *Manager) Stop() (err error) {
	...
	m.stopOnce.Do(func() {
		...
		m.gm.NeighborsEvents(gossip2.NeighborsGroupManual).NeighborRemoved.Detach(m.onGossipNeighborRemovedClosure)
		m.gm.NeighborsEvents(gossip2.NeighborsGroupManual).NeighborAdded.Detach(m.onGossipNeighborAddedClosure)
	})
	return err
}
...
func (m *Manager) removePeerByID(peerID identity.ID) error {
	...
	close(kp.removeCh)
	<-kp.doneCh
	if err := m.gm.DropNeighbor(peerID, gossip2.NeighborsGroupManual); err != nil && !errors.Is(err, gossip2.ErrUnknownNeighbor) {
	...
	for {
		if kp.getConnStatus() == ConnStatusDisconnected {
			...
			if kp.connDirection == ConnDirectionOutbound {
				err = m.gm.AddOutbound(ctx, kp.peer, gossip2.NeighborsGroupManual)
			} else if kp.connDirection == ConnDirectionInbound {
				err = m.gm.AddInbound(ctx, kp.peer, gossip2.NeighborsGroupManual, gossip2.WithNoDefaultTimeout())
			}
			if err != nil && !errors.Is(err, gossip2.ErrDuplicateNeighbor) && !errors.Is(err, context.Canceled) {
				m.log.Errorw(
...
func (m *Manager) onGossipNeighborRemoved(neighbor *gossip2.Neighbor) {
	m.changeNeighborStatus(neighbor, ConnStatusDisconnected)
}

func (m *Manager) onGossipNeighborAdded(neighbor *gossip2.Neighbor) {
...
func (m *Manager) changeNeighborStatus(neighbor *gossip2.Neighbor, connStatus ConnectionStatus) {
	m.knownPeersMutex.RLock()
	defer m.knownPeersMutex.RUnlock()
