...
type Manager struct {
	...
	onGossipNeighborRemovedClosure *event.Closure[*p2p2.NeighborRemovedEvent]
	onGossipNeighborAddedClosure   *event.Closure[*p2p2.NeighborAddedEvent]
}

// NewManager initializes a new Manager instance.
func NewManager(p2pm *p2p2.Manager, local *peer.Local, log *logger.Logger) *Manager {
	m := &Manager{
		p2pm:              p2pm,
		local:             local,
		log:               log,
	...
	m.onGossipNeighborRemovedClosure = event.NewClosure(func(event *p2p2.NeighborRemovedEvent) { m.onGossipNeighborRemoved(event.Neighbor) })
	m.onGossipNeighborAddedClosure = event.NewClosure(func(event *p2p2.NeighborAddedEvent) { m.onGossipNeighborAdded(event.Neighbor) })
	return m
}
...
func (m *Manager) Start() {
	m.startOnce.Do(func() {
		m.p2pm.NeighborGroupEvents(p2p2.NeighborsGroupManual).NeighborRemoved.Attach(m.onGossipNeighborRemovedClosure)
		m.p2pm.NeighborGroupEvents(p2p2.NeighborsGroupManual).NeighborAdded.Attach(m.onGossipNeighborAddedClosure)
		m.isStarted.Set()
	})
}
...
func (m *Manager) Stop() (err error) {
	...
	m.stopOnce.Do(func() {
		...
		m.p2pm.NeighborGroupEvents(p2p2.NeighborsGroupManual).NeighborRemoved.Detach(m.onGossipNeighborRemovedClosure)
		m.p2pm.NeighborGroupEvents(p2p2.NeighborsGroupManual).NeighborAdded.Detach(m.onGossipNeighborAddedClosure)
	})
	return err
}
...
func (m *Manager) removePeerByID(peerID identity.ID) error {
	...
	close(kp.removeCh)
	<-kp.doneCh
	if err := m.p2pm.DropNeighbor(peerID, p2p2.NeighborsGroupManual); err != nil && !errors.Is(err, p2p2.ErrUnknownNeighbor) {
	...
	for {
		if kp.getConnStatus() == ConnStatusDisconnected {
			...
			if kp.connDirection == ConnDirectionOutbound {
				err = m.p2pm.AddOutbound(ctx, kp.peer, p2p2.NeighborsGroupManual)
			} else if kp.connDirection == ConnDirectionInbound {
				err = m.p2pm.AddInbound(ctx, kp.peer, p2p2.NeighborsGroupManual, p2p2.WithNoDefaultTimeout())
			}
			if err != nil && !errors.Is(err, p2p2.ErrDuplicateNeighbor) && !errors.Is(err, context.Canceled) {
				m.log.Errorw(
...
func (m *Manager) onGossipNeighborRemoved(neighbor *p2p2.Neighbor) {
	m.changeNeighborStatus(neighbor, ConnStatusDisconnected)
}

func (m *Manager) onGossipNeighborAdded(neighbor *p2p2.Neighbor) {
...
func (m *Manager) changeNeighborStatus(neighbor *p2p2.Neighbor, connStatus ConnectionStatus) {
	m.knownPeersMutex.RLock()
	defer m.knownPeersMutex.RUnlock()
