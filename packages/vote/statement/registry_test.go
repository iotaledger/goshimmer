...
func TestRegistry(t *testing.T) {
	...
	v.AddConflict(Conflict{txA, Opinion{vote.Like, 1}})
	v.AddConflict(Conflict{txA, Opinion{vote.Like, 2}})
	v.AddConflict(Conflict{txB, Opinion{vote.Like, 1}})

	opinions := v.ConflictOpinion(txA)
	assert.Equal(t, 2, len(opinions))
	assert.Equal(t, Opinion{vote.Like, 2}, opinions.Last())
