...
// BookTransaction adds a new Transaction to the ledger state.
func (l *Ledgerstate) BookTransaction(transaction *Transaction) (targetBranchID BranchID, err error) {
	l.RLock()
	defer l.RUnlock()

	return l.UTXODAG.BookTransaction(transaction)
}
		...
		l.UTXODAG.CachedTransactionMetadata(currentTransactionID).Consume(func(transactionMetadata *TransactionMetadata) {
			...
			l.UTXODAG.CachedTransaction(currentTransactionID).Consume(func(transaction *Transaction) {
				for _, output := range transaction.Essence().Outputs() {
					...
					l.UTXODAG.CachedConsumers(output.ID()).Consume(func(consumer *Consumer) {
						transactionWalker.Push(consumer.TransactionID())
					})
				}
			})

			l.UTXODAG.Events().TransactionBranchIDUpdatedByMerge.Trigger(&TransactionBranchIDUpdatedByMergeEvent{
				TransactionID:    currentTransactionID,
				MergedBranchID:   branchID,
				BranchDAGUpdates: updatedBranches,
			})
		})
	}
