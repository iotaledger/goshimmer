...
func TestTangle_Flow(t *testing.T) {
	...
	tangle.Parser.Events.MessageRejected.AttachBefore(events.NewClosure(func(msgRejectedEvent *MessageRejectedEvent, _ error) {
		n := atomic.AddInt32(&rejectedMessages, 1)
		fmt.Printf("rejected by message filter messages %d/%d - %s\n", n, totalMsgCount, msgRejectedEvent.Message.ID())
	}))

	tangle.Parser.Events.MessageParsed.AttachBefore(events.NewClosure(func(msgParsedEvent *MessageParsedEvent) {
		n := atomic.AddInt32(&parsedMessages, 1)
		fmt.Printf("parsed messages %d/%d - %s\n", n, totalMsgCount, msgParsedEvent.Message.ID())
	}))

	// message invalid events
	tangle.Events.MessageInvalid.AttachBefore(events.NewClosure(func(messageID MessageID) {
		n := atomic.AddInt32(&invalidMessages, 1)
		fmt.Printf("invalid messages %d/%d - %s\n", n, totalMsgCount, messageID)
	}))

	tangle.Storage.Events.MessageStored.AttachBefore(events.NewClosure(func(messageID MessageID) {
		n := atomic.AddInt32(&storedMessages, 1)
		fmt.Printf("stored messages %d/%d - %s\n", n, totalMsgCount, messageID)
	}))
	...
	// decrease the counter when a missing message was received
	tangle.Storage.Events.MissingMessageStored.AttachBefore(events.NewClosure(func(messageID MessageID) {
		n := atomic.AddInt32(&missingMessages, -1)
		fmt.Printf("missing messages %d - %s\n", n, messageID)
	}))

	tangle.Solidifier.Events.MessageSolid.AttachBefore(events.NewClosure(func(messageID MessageID) {
		n := atomic.AddInt32(&solidMessages, 1)
		fmt.Printf("solid messages %d/%d - %s\n", n, totalMsgCount, messageID)
	}))

	tangle.Scheduler.Events.MessageScheduled.AttachBefore(events.NewClosure(func(messageID MessageID) {
		n := atomic.AddInt32(&scheduledMessages, 1)
		fmt.Printf("scheduled messages %d/%d - %s\n", n, totalMsgCount, messageID)
	}))

	tangle.Booker.Events.MessageBooked.AttachBefore(events.NewClosure(func(messageID MessageID) {
		n := atomic.AddInt32(&bookedMessages, 1)
		fmt.Printf("booked messages %d/%d - %s\n", n, totalMsgCount, messageID)
	}))

	tangle.ConsensusManager.Events.MessageOpinionFormed.AttachBefore(events.NewClosure(func(messageID MessageID) {
		n := atomic.AddInt32(&opinionFormedMessages, 1)
		fmt.Printf("opinion formed messages %d/%d\n", n, totalMsgCount)
	}))

	// data messages should not trigger this event
	tangle.LedgerState.UTXODAG.Events().TransactionConfirmed.AttachBefore(events.NewClosure(func(transactionID ledgerstate.TransactionID) {
		n := atomic.AddInt32(&opinionFormedTransactions, 1)
		fmt.Printf("opinion formed transaction %d/%d - %s\n", n, totalMsgCount, transactionID)
	}))

	tangle.Events.Error.AttachBefore(events.NewClosure(func(err error) {
		fmt.Printf("Error %s\n", err)
	}))
