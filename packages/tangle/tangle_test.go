...
func TestTangle_InvalidParentsAgeMessage(t *testing.T) {
	...
	newValidMessage := func(strongParents MessageIDs) *Message {
		message, err := NewMessage(emptyLikeReferencesFromStrongParents(strongParents), time.Now(), ed25519.PublicKey{}, 0, payload.NewGenericDataPayload([]byte("IsBooked")), 0, ed25519.Signature{}, 0, nil)
		assert.NoError(t, err)
		return message
	}
