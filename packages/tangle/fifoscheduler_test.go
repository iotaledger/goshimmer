...
func TestFifoScheduler(t *testing.T) {
	...
	messages["C"] = newTestParentsDataWithTimestamp("C", []MessageID{messages["A"].ID(), messages["B"].ID()}, []MessageID{}, nil, nil, time.Now())
	messages["D"] = newTestParentsDataWithTimestamp("D", []MessageID{messages["A"].ID(), messages["B"].ID()}, []MessageID{}, nil, nil, time.Now().Add(100*time.Millisecond))
	messages["E"] = newTestParentsDataWithTimestamp("E", []MessageID{messages["C"].ID(), messages["D"].ID()}, []MessageID{}, nil, nil, time.Now().Add(3*time.Second))
