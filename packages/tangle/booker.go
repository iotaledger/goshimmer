...
func (b *Booker) PayloadBranchIDs(messageID MessageID) (branchIDs ledgerstate.BranchIDs, err error) {
	...
	b.tangle.Storage.Message(messageID).Consume(func(message *Message) {
		...
		b.tangle.LedgerState.TransactionMetadata(transaction.ID()).Consume(func(transactionMetadata *ledgerstate.TransactionMetadata) {
			resolvedConflictBranchIDs, resolveErr := b.tangle.LedgerState.ResolvePendingConflictBranchIDs(ledgerstate.NewBranchIDs(transactionMetadata.CompressedBranchesID()))
			if resolveErr != nil {
...
// bookPayload books the Payload of a Message and returns its assigned CompressedBranchesID.
...
// PropagateForkedBranch propagates the forked CompressedBranchesID to the future cone of the attachments of the given Transaction.
...
// propagateForkedTransactionToMarkerFutureCone propagates a newly created CompressedBranchesID into the future cone of the given Marker.
...
// forkSingleMarker propagates a newly created CompressedBranchesID to a single marker and queues the next elements that need to be
// visited.
func (b *Booker) forkSingleMarker(currentMarker *markers.Marker, newBranchID ledgerstate.BranchID, markerWalker *walker.Walker) (err error) {
	// update CompressedBranchesID mapping
	...
	// propagate updates to later CompressedBranchesID mappings of the same sequence.
...
// propagateForkedTransactionToMetadataFutureCone updates the future cone of a Message to belong to the given conflict CompressedBranchesID.
...
type BookerEvents struct {
	...
	// MessageBranchUpdated is triggered when the CompressedBranchesID of a Message is changed in its MessageMetadata.
	MessageBranchUpdated *events.Event

	// MarkerBranchAdded is triggered when a Marker is mapped to a new CompressedBranchesID.
