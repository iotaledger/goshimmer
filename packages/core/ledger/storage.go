...
func newStorage(ledger *Ledger) (storage *Storage) {
	storage = &Storage{
		transactionStorage: objectstorage.NewInterfaceStorage[utxo.Transaction](
			objectstorage.NewStoreWithRealm(ledger.options.optsStore, database.PrefixLedger, PrefixTransactionStorage),
			transactionFactory(ledger.options.optsVM),
			ledger.options.optsCacheTimeProvider.CacheTime(ledger.options.optsTransactionCacheTime),
			objectstorage.LeakDetectionEnabled(false),
			objectstorage.StoreOnCreation(true),
		),
		transactionMetadataStorage: objectstorage.NewStructStorage[TransactionMetadata](
			objectstorage.NewStoreWithRealm(ledger.options.optsStore, database.PrefixLedger, PrefixTransactionMetadataStorage),
			ledger.options.optsCacheTimeProvider.CacheTime(ledger.options.optTransactionMetadataCacheTime),
			objectstorage.LeakDetectionEnabled(false),
		),
		outputStorage: objectstorage.NewInterfaceStorage[utxo.Output](
			objectstorage.NewStoreWithRealm(ledger.options.optsStore, database.PrefixLedger, PrefixOutputStorage),
			outputFactory(ledger.options.optsVM),
			ledger.options.optsCacheTimeProvider.CacheTime(ledger.options.optsOutputCacheTime),
			objectstorage.LeakDetectionEnabled(false),
			objectstorage.StoreOnCreation(true),
		),
		outputMetadataStorage: objectstorage.NewStructStorage[OutputMetadata](
			objectstorage.NewStoreWithRealm(ledger.options.optsStore, database.PrefixLedger, PrefixOutputMetadataStorage),
			ledger.options.optsCacheTimeProvider.CacheTime(ledger.options.optsOutputMetadataCacheTime),
			objectstorage.LeakDetectionEnabled(false),
		),
		consumerStorage: objectstorage.NewStructStorage[Consumer](
			objectstorage.NewStoreWithRealm(ledger.options.optsStore, database.PrefixLedger, PrefixConsumerStorage),
			ledger.options.optsCacheTimeProvider.CacheTime(ledger.options.optsConsumerCacheTime),
			objectstorage.LeakDetectionEnabled(false),
