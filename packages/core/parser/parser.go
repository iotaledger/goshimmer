...
func (p *Parser) Parse(blockBytes []byte, source *peer.Peer) {
	p.bytesFilters[0].Filter(blockBytes, source)
}
...
func (p *Parser) parseBlock(bytes []byte, source *peer.Peer) {
	...
	_ = blk.DetermineID()
	p.blockFilters[0].Filter(blk, source)
}
...
func (f *BlockSignatureFilter) Filter(blk *models.Block, p *peer.Peer) {
	...
	f.getRejectCallback()(blk, ErrInvalidSignature, p)
}
