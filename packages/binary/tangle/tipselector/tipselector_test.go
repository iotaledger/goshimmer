...
func Test(t *testing.T) {
	...
	transaction1 := transaction.New(trunk1, branch1, localIdentity1.PublicKey(), time.Now(), 0, data.New([]byte("testtransaction")), localIdentity1)
	...
	transaction2 := transaction.New(transaction.EmptyId, transaction.EmptyId, localIdentity2.PublicKey(), time.Now(), 0, data.New([]byte("testtransaction")), localIdentity2)
	...
	transaction3 := transaction.New(trunk3, branch3, localIdentity3.PublicKey(), time.Now(), 0, data.New([]byte("testtransaction")), localIdentity3)
