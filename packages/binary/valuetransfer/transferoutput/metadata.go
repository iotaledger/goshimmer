...
// Metadata contains the information of a transfer output, that are based on our local perception of things (i.e. if it
...
// Id return the id of the TransferOutput that this Metadata is associated to.
func (metadata *Metadata) Id() Id {
	return metadata.id
}

// Solid returns true if the TransferOutput has been marked as solid.
...
// SetSolid marks a TransferOutput as either solid or not solid.
...
// SoldificationTime returns the time when the TransferOutput was marked to be solid.
...
func (metadata *Metadata) String() string {
	return stringify.Struct("transferoutput.Metadata",
