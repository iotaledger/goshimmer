...
func configureManaFeed() {
	manaFeedWorkerPool = workerpool.NewNonBlockingQueuedWorkerPool(func(task workerpool.Task) {
		...
		case MsgTypeManaPledge:
			sendManaPledge(task.Param(1).(*mana2.PledgedEvent))
		case MsgTypeManaRevoke:
			sendManaRevoke(task.Param(1).(*mana2.RevokedEvent))
		}
		task.Return(nil)
...
func runManaFeed() {
	notifyManaPledge := event.NewClosure(func(ev *mana2.PledgedEvent) {
		manaFeedWorkerPool.TrySubmit(MsgTypeManaPledge, ev)
	})
	notifyManaRevoke := event.NewClosure(func(ev *mana2.RevokedEvent) {
	...
	if err := daemon.BackgroundWorker("Dashboard[ManaUpdater]", func(ctx context.Context) {
		mana2.Events.Pledged.Attach(notifyManaPledge)
		mana2.Events.Revoked.Attach(notifyManaRevoke)
...
func sendManaValue() {
	...
	if err != nil && !errors.Is(err, mana2.ErrNodeNotFoundInBaseManaVector) && !errors.Is(err, manaPlugin.ErrQueryNotAllowed) {
...
func sendManaMapOverall() {
	accessManaList, _, err := manaPlugin.GetHighestManaNodes(mana2.AccessMana, 0)
	...
	accessPayload := &ManaNetworkListBlkData{ManaType: mana2.AccessMana.String()}
	totalAccessMana := 0.0
	...
	consensusManaList, _, err := manaPlugin.GetHighestManaNodes(mana2.ConsensusMana, 0)
	if err != nil && !errors.Is(err, manaPlugin.ErrQueryNotAllowed) {
		log.Errorf("failed to get list of n highest consensus mana nodes: %s ", err.Error())
	}
	consensusPayload := &ManaNetworkListBlkData{ManaType: mana2.ConsensusMana.String()}
...
func sendManaMapOnline() {
	accessManaList, _, err := manaPlugin.GetOnlineNodes(mana2.AccessMana)
	if err != nil && !errors.Is(err, manaPlugin.ErrQueryNotAllowed) {
...
func sendManaPledge(ev *mana2.PledgedEvent) {
...
func sendManaRevoke(ev *mana2.RevokedEvent) {
	ManaBufferInstance().StoreEvent(ev)
	broadcastWsBlock(&wsblk{
...
func sendAllowedManaPledge(ws *websocket.Conn) error {
	allowedAccess := manaPlugin.GetAllowedPledgeNodes(mana2.AccessMana)
	allowedConsensus := manaPlugin.GetAllowedPledgeNodes(mana2.ConsensusMana)
...
type ManaNetworkListBlkData struct {
	...
	Nodes     []mana2.NodeStr `json:"nodes"`
}
