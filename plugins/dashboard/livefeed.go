...
func runLiveFeed() {
	...
	if err := daemon.BackgroundWorker("Dashboard[BlkUpdater]", func(ctx context.Context) {
		deps.Protocol.Events.InstanceManager.Instance.Engine.Tangle.BlockDAG.BlockAttached.Attach(notifyNewBlk)
		<-ctx.Done()
		log.Info("Stopping Dashboard[BlkUpdater] ...")
		deps.Protocol.Events.InstanceManager.Instance.Engine.Tangle.BlockDAG.BlockAttached.Detach(notifyNewBlk)
