...
The server instance of the web API is contacted via `webapi.Server` and two methods are currently employed:

First, with `GET` we register a new GET route for a handler function. The handler is accessed via the address `path`. The handler for a GET method can set the node to perform certain actions.
```
webapi.Server().GET("path", Handler)
```	
...
```
webapi.Server().POST("path", Handler)
```	
...
```
can be sent to `http://127.0.0.1:8080/data`, which will issue a data message containing "HelloWor" (note that this is a very limited example where the data input is size limited.)
