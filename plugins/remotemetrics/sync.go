...
func checkSynced() {
	...
	if oldTangleTimeSynced != tts {
		...
		syncStatusChangedEvent := &remotemetrics.TangleTimeSyncChangedEvent{
			...
			Time:           time.Now(),
			ATT:            deps.Protocol.Engine().Clock.Acceptance().Time(),
			RATT:           deps.Protocol.Engine().Clock.Acceptance().RelativeTime(),
			CTT:            deps.Protocol.Engine().Clock.Confirmation().Time(),
			RCTT:           deps.Protocol.Engine().Clock.Confirmation().RelativeTime(),
			CurrentStatus:  tts,
