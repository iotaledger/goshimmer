...
func sendMessageSchedulerRecord(messageID tangle.MessageID, recordType string) {
	...
	deps.Tangle.Storage.Message(messageID).Consume(func(message *tangle.Message) {
		...
		deps.Tangle.Storage.MessageMetadata(messageID).Consume(func(messageMetadata *tangle.MessageMetadata) {
			...
			record.GradeOfFinality = uint8(messageMetadata.ConfirmationState())
			record.GradeOfFinalityTimestamp = messageMetadata.ConfirmationStateTime()
			if !messageMetadata.ConfirmationStateTime().IsZero() {
				record.DeltaGradeOfFinalityTime = messageMetadata.ConfirmationStateTime().Sub(record.IssuedTimestamp).Nanoseconds()
			}
