...
func TestApis(t *testing.T) {
	...
	require.NoError(t, err)
	assert.Equal(t, 3, len(resp3.Nodes))
	assert.Equal(t, 3, len(resp4.Nodes))
	for i := 0; i < 3; i++ {
	...
	require.NoError(t, err)
	assert.Equal(t, 3, len(resp6.Online))
	...
	assert.Equal(t, base58.Encode(peers[2].ID().Bytes()), resp6.Online[1].ID)
	assert.Equal(t, base58.Encode(peers[0].ID().Bytes()), resp7.Online[1].ID)
	...
	for i := 0; i < len(peers); i++ {
		...
		require.True(t, ok)
		if i == 0 {
			assert.Equal(t, 4, len(logs.Pledge))
