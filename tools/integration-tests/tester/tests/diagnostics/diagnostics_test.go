...
func TestDiagnosticApis(t *testing.T) {
	...
	fmt.Println("run snapshottool/diagnostic/messages/firstweakreferences")
	resp, err = peers[0].GoShimmerAPI.GetDiagnosticsFirstWeakMessageReferences()
	require.NoError(t, err, "error while performing snapshottool/diagnostic/messages/firstweakreferences api call")
	records, err = resp.ReadAll()
	...
	fmt.Println("run snapshottool/diagnostic/tips")
	tips, err := peers[0].GoShimmerAPI.GetDiagnosticsTips()
	require.NoError(t, err, "error while performing snapshottool/diagnostic/tips api call")
	records, err = tips.ReadAll()
	require.NoError(t, err, "error while reading snapshottool/diagnostic/tips api csv")
	require.Equal(t, tipsHeader, records[0], "unexpected tips header")

	fmt.Println("run snapshottool/diagnostic/branches")
	branches, err := peers[0].GoShimmerAPI.GetDiagnosticsBranches()
	require.NoError(t, err, "error while running snapshottool/diagnostic/branches")
	records, err = branches.ReadAll()
	require.NoError(t, err, "error while reading snapshottool/diagnostic/branches csv")
	require.Equal(t, branchesHeader, records[0], "unexpected branches header")

	fmt.Println("run snapshottool/diagnostic/utxodag")
	dag, err := peers[0].GoShimmerAPI.GetDiagnosticsUtxoDag()
	require.NoError(t, err, "error while running snapshottool/diagnostic/utxodag api call")
	records, err = dag.ReadAll()
	require.NoError(t, err, "error while reading snapshottool/diagnostic/utxodag csv")
	require.Equal(t, utxoDagHeader, records[0], "unexpected utxoDagHeader header")

	fmt.Println("run snapshottool/diagnostic/drng")
	drng, err := peers[0].GoShimmerAPI.GetDiagnosticsDRNG()
	require.NoError(t, err, "error while running snapshottool/diagnostic/drng api call")
	records, err = drng.ReadAll()
	require.NoError(t, err, "error while reading snapshottool/diagnostic/drng csv")
