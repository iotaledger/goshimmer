...
func TestOrphanageTSC(t *testing.T) {
	...
	blocksToOrphan := tests.SendDataMessagesWithDelay(t, partition1, 150, delayBetweenDataMessages)
	t.Logf("Sending %d data blocks on majority partition", 50)
	blocksToConfirm := tests.SendDataMessages(t, partition2, 50)
	...
	tests.SendDataMessagesWithDelay(t, n.Peers(), 150, delayBetweenDataMessages)

	tests.RequireMessagesAvailable(t, n.Peers(), blocksToConfirm, time.Minute, tests.Tick, gof.High)
	tests.RequireMessagesOrphaned(t, partition1, blocksToOrphan, time.Minute, tests.Tick)
}
