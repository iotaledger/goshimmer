...
wget http://localhost:8080/snapshot?index=4 -O TEST_snapshot.bin

docker run -it --rm -v $PWD/TEST_snapshot.bin:/app/snapshot.bin --network docker-network_goshimmer --name joiner \
  -p 8011:8081 \
